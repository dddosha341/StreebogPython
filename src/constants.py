"""
Константы алгоритма Стрибог (ГОСТ 34.11-2018 / RFC 6986).

Все значения взяты напрямую из стандарта:
- PI (π): таблица нелинейной подстановки (S-box), 256 элементов.
- TAU (τ): перестановка байтов, 64 элемента.
- A: 64 строки матрицы линейного преобразования L в GF(2), каждая — 64-битное целое.
- C: 12 итерационных констант для функции E, каждая — 64 байта.
- IV_512, IV_256: начальные значения для 512- и 256-битных вариантов.

Порядок байтов:
- Внутреннее представление состояния — массив из 64 байтов.
- Для арифметических операций (счётчик N, контрольная сумма Σ) используется little-endian.
- Константы L (массив A) хранятся как 64-битные целые числа в little-endian формате,
  т.е. бит 0 (LSB) первого байта 8-байтовой группы соответствует A[k*8 + 0].
"""

from typing import Tuple

# =============================================================================
# S-box (π) — нелинейная подстановка, 256 элементов
# ГОСТ 34.11-2018, раздел 5.1 / RFC 6986, Section 11.1
# =============================================================================
PI: Tuple[int, ...] = (
...
# =============================================================================
# Перестановка τ (TAU) — транспозиция 8×8 матрицы байтов
# ГОСТ 34.11-2018, раздел 5.2
# τ(i) = (i mod 8) * 8 + (i div 8), для i = 0..63
# Это эквивалентно транспозиции матрицы 8×8, записанной по строкам.
# =============================================================================
TAU: Tuple[int, ...] = (
...
# =============================================================================
# Матрица линейного преобразования L — 64 элемента, каждый 64-бит
# ГОСТ 34.11-2018, раздел 5.3 / RFC 6986
#
# Линейное преобразование L обрабатывает состояние как 8 независимых
# 64-битных слов. Для каждого слова v (8 байтов, little-endian):
#   result = 0
#   for j in range(64):
#       if bit j of v is set:
#           result ^= A[j]
# где бит 0 — это LSB первого байта.
#
# ВАЖНО: значения A приведены в порядке, соответствующем LE-нумерации битов
# (бит 0 = LSB байта 0 → A[0], бит 7 = MSB байта 0 → A[7],
#  бит 8 = LSB байта 1 → A[8], и т.д.).
# Это НЕ совпадает с порядком записи в тексте RFC, где бит 0 — старший бит.
# =============================================================================
A: Tuple[int, ...] = (
...
# =============================================================================
# Итерационные константы C[1]..C[12] для функции шифрования E
# ГОСТ 34.11-2018 / RFC 6986
# Каждая константа — 64 байта (512 бит).
#
# ВАЖНО: RFC/ГОСТ записывает константы в big-endian (MSB first), но внутри
# алгоритма все операции выполняются над байтовыми массивами в little-endian
# представлении. Поэтому здесь константы хранятся в LE-порядке
# (развёрнуты по сравнению с текстом стандарта).
# =============================================================================
C: Tuple[bytes, ...] = (
    # C[1]
    bytes((
...
# =============================================================================
IV_512: bytes = b'\x00' * 64  # Streebog-512: h = 0^512
IV_256: bytes = b'\x01' * 64  # Streebog-256: h = (00000001)^64

# =============================================================================
# Вспомогательная константа: 512 как 64-байтовое LE-число
# Используется для инкремента счётчика N на каждый обработанный блок.
# 512 = 0x200, в little-endian: 0x00, 0x02, 0x00, ..., 0x00
# =============================================================================
BLOCK_SIZE: int = 64  # размер блока в байтах (512 бит)
N_BLOCK_BITS: bytes = (512).to_bytes(64, byteorder='little')
